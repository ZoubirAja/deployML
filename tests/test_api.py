...
def test_predict_valide(client, employe_valide):
    ...
    response = client.post("/predict_nouveau", json=employe_valide, headers=headers)
...
def test_predict_champ_manquant(client):
    ...
    response = client.post("/predict_nouveau", json={"age": 28}, headers=headers)  # plein de champs manquants
...
def test_predict_by_id_valide(client):
    ...
    response = client.post("/predict/1", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "prediction" in data
...
def test_predict_by_id_inexistant(client):
    ...
    response = client.post("/predict/999999", headers=headers)
